...
//    int   NumDisr; //***** ROB: Geoff's disruption counter (24-03-20) *****
...
#endif //COMPUT_SPECPHOT_PROPERTIES
#ifndef LITE_OUTPUT
    ...
    float sfh_MetalsDiskMassRings[RNUM][SFH_NBIN][NUM_METAL_CHANNELS]; //Metals locked up in stars in disk.
    float sfh_MetalsBulgeMassRings[RNUM][SFH_NBIN][NUM_METAL_CHANNELS]; //Metals locked up in stars in bulge.
...
#ifdef OUTPUT_RINGS_EXTRA
    float sfh_DiskMass_elementsRings[RNUM][SFH_NBIN][NUM_ELEMENTS];
    float sfh_BulgeMass_elementsRings[RNUM][SFH_NBIN][NUM_ELEMENTS];
#endif //OUTPUT_RINGS_EXTRA
...
#else //H2_AND_RINGS
  float f_cmax[NUM_ELEMENTS]; // unitless // max fraction of molecular media //ROB: I think this is actually the maximum mass fraction of element ee in clouds that is in dust.
#endif //H2_AND_RINGS
