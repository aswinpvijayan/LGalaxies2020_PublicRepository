...
void starformation(int p, int centralgal, double time, double dt, int nstep)
{
  ...
  for(j=0;j<RNUM;j++)
    {
      ...
      //ROB: (05-06-23) I added this to catch instances where strdotr cannot be calculated (e.g. due to sfe being inf, likely because tdyn = 0.0):
      if ((isnormal(strdotr[j]) == 0) || (strdotr[j] < 0.0)) { //i.e. if strdotr[j] is inf or nan or negative
    	  strdotr[j] = 0.0;
      }
    }
  ...
  for (j=0,strdot=0;j<RNUM;j++) {
	  /*
	  if(strdotr[j] < 0.0)
		  printf("\n***** Neg: strdotr = %e (%i) | sfe = %e (%i) | strdot = %e (%i)\n", strdotr[j], isnormal(strdotr[j]), sfe, isinf(sfe), strdot+strdotr[j], isnan(strdot+strdotr[j]));
	  if(strdotr[j] != strdotr[j])
		  printf("\n***** Nan: strdotr = %e (%i) | sfe = %e (%i) | strdot = %e (%i)\n", strdotr[j], isnormal(strdotr[j]), sfe, isinf(sfe), strdot+strdotr[j], isnan(strdot+strdotr[j]));
	  */
	  strdot+=strdotr[j];
  }
  ...
  for(j=0;j<RNUM;j++)
    {
      if(strdotr[j] < 0.0) //ROB: Shouldn't this correction be done above, before strdot is defined as the sum of strdotr[j]?? <-- I have not added this check above accordingly. (05-06-23)
    	  strdotr[j] = 0.;
  ...
  //ROB: (05-06-23) Added these checks to make sure that ColdGas isn't nan, inf, or negative before assigning its value to stars:
  if(stars > Gal[p].ColdGas) {
	  if (isnormal(Gal[p].ColdGas) == 0 || Gal[p].ColdGas < 0.0)
		  stars = 0.0;
	  	  //printf("\n***** stars = %e | Gal[p].ColdGas = %e\n", stars, Gal[p].ColdGas);
	  else
		  stars = Gal[p].ColdGas;
  }
  ...
  for(j=0;j<RNUM;j++)
    if(starsRings[j]>Gal[p].ColdGasRings[j]) {
    	if (isnormal(Gal[p].ColdGasRings[j]) == 0 || Gal[p].ColdGasRings[j] < 0.0)
    		starsRings[j] = 0.0;
    		//printf("\n***** starsRings[j] = %e | Gal[p].ColdGasRings[j] = %e\n", starsRings[j], Gal[p].ColdGasRings[j]);
    	else
    		starsRings[j] = Gal[p].ColdGasRings[j];
    }
#endif //H2_AND_RINGS
...
/* if update_stars_due_to_reheat is commented out, uncomment this lines:
 ...
 #ifdef H2_AND_RINGS
   double SfrRingsTot = 0.0;
   for(j=0;j<RNUM;j++)
   {
     Gal[p].SfrRings[j] += starsRings[j] / (dt * STEPS);
     Gal[p].SfrInstRings[j] = starsRings[j] / dt; //*****ROB*****//
     SfrRingsTot += starsRings[j] / (dt * STEPS);
   }
 #endif

 if (((stars/(dt*STEPS)) / SfrRingsTot < 0.99999) || ((stars/(dt*STEPS)) / SfrRingsTot > 1.00001))
	 printf("\nmodel_starformation_and_feedback.c: Sfr_added = %e | SfrRingsTot_added = %e\n", stars/(dt*STEPS), SfrRingsTot);
...
#ifndef DUST_HOTGAS //No dust is destroyed just by being moved to the HotGas when DUST_HOTGAS is on.
    	for(ee=0;ee<NUM_ELEMENTS;ee++)
...
#ifndef DUST_HOTGAS //No dust is destroyed just by being moved to the HotGas when DUST_HOTGAS is on.
	  for(ee=0;ee<NUM_ELEMENTS;ee++)
		  Gal[p].DustColdGasRates[4] += (Gal[p].DustColdGasDiff_elements[ee]*fractionRings + Gal[p].DustColdGasClouds_elements[ee]*fractionRings)/(deltaT * UnitTime_in_years);
