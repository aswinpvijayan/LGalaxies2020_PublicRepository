 ...
 //STAR FORMATION HISTORIES / RATES
  ...
  for (j=0;j<=o->sfh_ibin;j++) {
 	 ...
 	 for(ll=0; ll<NUM_METAL_CHANNELS; ll++)
	 {
...
#ifdef OUTPUT_RINGS_EXTRA
 	 for(ll=0; ll<RNUM; ll++) {
 		for(int mm=0; mm<NUM_METAL_CHANNELS; mm++) {
 			o->sfh_MetalsDiskMassRings[ll][j][mm]=g->sfh_MetalsDiskMassRings[ll][j][mm]; //Metals locked up in stars in disk.
 			o->sfh_MetalsBulgeMassRings[ll][j][mm]=g->sfh_MetalsBulgeMassRings[ll][j][mm]; //Metals locked up in stars in bulge.
 		}
 	 }
 	 ...
 	 for(ll=0; ll<RNUM; ll++) {
 		for(kk=0;kk<NUM_ELEMENTS;kk++) {
 			o->sfh_DiskMass_elementsRings[ll][j][kk]=g->sfh_DiskMass_elementsRings[ll][j][kk];
 			o->sfh_BulgeMass_elementsRings[ll][j][kk]=g->sfh_BulgeMass_elementsRings[ll][j][kk];
 		}
 	 }
#endif
 	 for(kk=0;kk<NUM_ELEMENTS;kk++) {
...
// END DEBUG
  ...
  for (j=o->sfh_ibin+1;j<SFH_NBIN;j++) {
 	 ...
 	 for(ll=0; ll<RNUM; ll++) {
 		for(int mm=0; mm<NUM_METAL_CHANNELS; mm++) {
 			o->sfh_MetalsDiskMassRings[ll][j][mm]=0.;
 			o->sfh_MetalsBulgeMassRings[ll][j][mm]=0.;
 		}
 	 }
#endif
	 for(ll=0; ll<NUM_METAL_CHANNELS; ll++) {
 	 ...
 	 for(ll=0; ll<RNUM; ll++) {
 		for(kk=0;kk<NUM_ELEMENTS;kk++) {
 			o->sfh_DiskMass_elementsRings[ll][j][kk]=0.;
 			o->sfh_BulgeMass_elementsRings[ll][j][kk]=0.;
 		}
 	 }
#endif
 	for(kk=0;kk<NUM_ELEMENTS;kk++) {
...
#ifdef H2_AND_RINGS
  for(ll=0; ll<RNUM; ll++)
	  o->t_des[ll] = g->t_des[ll];
#else //H2_AND_RINGS
  o->t_des = g->t_des;
#endif //H2_AND_RINGS
  //o->t_sput = g->t_sput;
#ifdef DUST_HOTGAS
  o->t_sput_HotGas = g->t_sput_HotGas;
#endif //DUST_HOTGAS
#ifdef DUST_EJECTEDMASS
  o->t_sput_EjectedMass = g->t_sput_EjectedMass;
#endif //DUST_EJECTEDMASS
  //o->DustColdGasRates = g->DustColdGasRates;
  for (int dd=0;dd<NUM_COLDGAS_DUST_RATES;dd++)
	  o->DustColdGasRates[dd] = g->DustColdGasRates[dd];
#ifdef DUST_HOTGAS
  for (int dd=0;dd<NUM_HOTGAS_DUST_RATES;dd++)
	  o->DustHotGasRates[dd] = g->DustHotGasRates[dd];
#endif //DUST_HOTGAS
#ifdef DUST_EJECTEDMASS
  o->DustEjectedMassRates = g->DustEjectedMassRates;
#endif //DUST_EJECTEDMASS
#endif //FULL_DUST_RATES
#ifdef TAU_RINGS
  for(ll=0; ll<RNUM; ll++)
	  o->t_acc[ll] = g->t_acc[ll];
#else //TAU_RINGS
  o->t_acc = g->t_acc;
#endif //TAU_RINGS
  for(kk=0;kk<NUM_ELEMENTS;kk++) {
	  ...
	  for(ll=0; ll<RNUM; ll++)
		  o->f_cmax[ll][kk] = g->f_cmax[ll][kk];
#endif //OUTPUT_DUST_RINGS
#else //H2_AND_RINGS
	  o->f_cmax[kk] = g->f_cmax[kk];
#endif
...
#endif //DUST_HOTGAS
#ifdef DUST_EJECTEDMASS
	  o->DustEjectedMass_elements[kk] = g->DustEjectedMass_elements[kk];
#endif //DUST_EJECTEDMASS
#ifdef OUTPUT_DUST_RINGS
      for(ll=0; ll<RNUM; ll++) {
  ...
  o->DiskSNIIRate = CORRECTDBFLOAT(g->DiskSNIIRate / UnitTime_in_s * SEC_PER_YEAR); //in yr^-1 //N.B. this is equivalent to DiskSNIIRate * (SEC_PER_YEAR / UnitTime_in_s), which converts [1/time_code-units] --> [1/yr]
  //o->DiskSNIIRate = CORRECTDBFLOAT(g->DiskSNIIRate / (UnitTime_in_years/Hubble_h)); //in yr^-1
  ...
  o->DiskSNIaRate = CORRECTDBFLOAT(g->DiskSNIaRate / UnitTime_in_s * SEC_PER_YEAR); //in yr^-1
  o->BulgeSNIaRate = CORRECTDBFLOAT(g->BulgeSNIaRate  / UnitTime_in_s * SEC_PER_YEAR); //in yr^-1
  o->ICMSNIaRate = CORRECTDBFLOAT(g->ICMSNIaRate  / UnitTime_in_s * SEC_PER_YEAR); //in yr^-1
#ifndef LITE_OUTPUT
